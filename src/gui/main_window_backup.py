...
class MainWindow:
    ...
    def _create_scanner_tab(self, parent):
        ...
        # Master list section
        master_list_card = tk.Frame(right_panel, bg=THEME_COLORS['surface'], 
                                   relief='solid', borderwidth=1)
        master_list_card.pack(fill=tk.X, pady=(0, 10))
        
        master_list_title = tk.Label(master_list_card, text="Master List", font=HEADER_FONT,
                                    fg=THEME_COLORS['text'], bg=THEME_COLORS['surface'])
    ...
    def _create_history_tab(self, parent):
        ...
        self.history_tree.column('Content', width=400)
    ...
    def _clear_history(self):
        ...
        self.app_manager.clear_last_scan()
    ...
    def _copy_from_history(self, event):
        ...
        if selection:
            ...
            if self.app_manager.copy_to_clipboard(content):
                self.update_status("Selected item copied to clipboard")
                messagebox.showinfo("Copied", "Selected item copied to clipboard!")
        ...
        if filename:
            try:
                with open(filename, 'w') as f:
                    f.write("Time,Content,Type\n")
                    for timestamp, content, barcode_type in self.scan_history:
                        f.write(f"{timestamp},{content},{barcode_type}\n")
    ...
    def process_scan(self, data, barcode_type):
        ...
        self.scan_history.append((timestamp, data, barcode_type))
        self.history_tree.insert('', 0, values=(timestamp, data, barcode_type))
